...
func (suite *ProviderTestSuite) TestPrometheusFiltering() {
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			prometheus.ParseMetricsWithFilterFunc = func(data []byte, filter []string, whitelist map[string]struct{}) ([]prom.MetricFamily, error) {
				...
				metrics, err := prom.ParseMetricsWithFilter(data, filter, whitelist)
				var found bool
